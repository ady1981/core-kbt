...
- интеграция с MCP-серверами инструментов
- кеширование ответов от LLM tools (инструментов) для сохранения контекста, с возможностью перевычисления запроса другими моделями (TODO)
- агрегированный LLM-запрос: сбор контекста для ответа через список MCP-профилей по списку пользователя, генерация целевых ответов LLM моделями из списка пользователя и объединение всех равнозначных ответов в агрегированный (объединенный) ответ (TODO) 
...
- реализация конкретных ИИ-функций: группы развития мета-знаний и знаний
...
```json lines
{
  ...
  "tools_model": 'google/gemini-flash' // Опционально
}
```
