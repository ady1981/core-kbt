...
def merge_arrays(model, left, right, array_item_json_schema, merge_strategy):
    equivalence_items = ai_function_template.evaluate('arrays_semantic_equivalence', with_model_input_data({'array_1': left, 'array_2': right, 'item_json_schema': array_item_json_schema}, model))

    upd_items = []

    # not_comparable
    not_comparable = equivalence_items.get('not_comparable', {})
    ## TODO: outside log configuration
    log_str('not_comparable:\n' + dump_json(not_comparable))
    ...
    # comparable.equal
    comparable_equal = equivalence_items.get('comparable', {}).get('equal', [])
    log_str('comparable_equal:\n' + dump_json(comparable_equal))
    upd_items += [c['_1'][1] for c in comparable_equal]

    # comparable.not_equal
    comparable_not_equal = equivalence_items.get('comparable', {}).get('not_equal', {})
    log_str('comparable_not_equal:\n' + dump_json(comparable_not_equal))
...
def rewrite_items_by_examples(model, items, example_items, array_item_json_schema):
    item_description = calc_context(array_item_json_schema)
    context_knowledge_specification = f'### Item description\n{item_description}'
    r = [ai_function_template.evaluate('rewrite', with_model_input_data({
        'item': c,
